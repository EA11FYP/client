...
const index = ({ id, auth, history }) => {
  ...
  let fetchPlacement = async () => {
    let response = await fetch(
      `${process.env.REACT_APP_DOMAIN}/api/placement/one/${id}`,
      {
        method: "get",
      }
    );
    let res = await response.json();
    if (response.status === 200 && res) setPlacementDetails(res);
  };

  let username = placementDetails.author
    ? placementDetails.author.username
    : null;
  let ids = placementDetails.author ? placementDetails.author.id : "123";

  useEffect(() => {
    if (auth && ids === auth._id) setShowBtn(true);
  ...
  let editPostHandler = async (event) => {
    ...
    let response = await fetch(
      `${process.env.REACT_APP_DOMAIN}/api/placement/edit/${id}`,
      {
        method: "post",
        body,
        headers: {
  ...
  let deletePostHandler = async (event) => {
    ...
    setIsLoading(true);
    let response = await fetch(
      `${process.env.REACT_APP_DOMAIN}/api/placement/delete/${id}`,
      {
        method: "delete",
      }
    );
    ...
    <React.Fragment>
      <div className="forumView-edit">
        <form onSubmit={editPostHandler}>
          ...
          <input
            className="forumView-edit-input"
            required
            value={updatedTitle}
          ...
          <textarea
            className="forumView-edit-textarea"
            required
            value={updatedBody}
            cols="15"
  ...
  return (
    <div>
      <a href="/placement/home" style={{ padding: 25 }}>
        <strong>Back</strong>
      </a>
      <div className="placementView-body">
        ...
        <p className="placementView-body-domain">
          <b>CTC</b>: INR {placementDetails.ctc}
        </p>
        ...
        <p className="placementView-body-description">
          {placementDetails.body}
        </p>
        <p className="placementView-body-author">{`created by ${username}`}</p>
        <p className="placementView-body-author">
          <strong>{placementDetails.date}</strong>
        </p>
      </div>
      {showBtn && (
